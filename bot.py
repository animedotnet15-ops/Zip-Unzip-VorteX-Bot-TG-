...
def _api_base() -> str:
    return config.local_bot_api_base or "https://api.telegram.org"


def render_bar(percent: int, width: int = 16) -> str:
    percent = max(0, min(100, percent))
    filled = int(width * percent / 100)
    return "◼" * filled + "◻" * (width - filled)


def progress_text(icon: str, label: str, percent: int) -> str:
    return f"{icon} {label}\n{render_bar(percent)} {percent}%"


async def _safe_edit(status_msg: Message, text: str) -> None:
    try:
...
async def download_with_progress(file_id: str, dest: Path, status_msg: Message, label: str) -> bool:
    try:
        file = await bot.get_file(file_id)
    except TelegramBadRequest as e:
        LOG.warning(f"get_file failed (file may exceed Bot API size limit): {e}")
        return False

    total = file.file_size or 0
    url = f"{_api_base()}/file/bot{config.bot_token}/{file.file_path}"
    downloaded = 0
    last_update = 0.0
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=None)) as resp:
                if resp.status != 200:
                    LOG.warning(f"Download HTTP {resp.status} for {file_id}")
                    return False
                with dest.open("wb") as f:
                    async for chunk in resp.content.iter_chunked(512 * 1024):
                        ...
                        now = time.time()
                        if total and (now - last_update > 1.5 or downloaded >= total):
                            last_update = now
                            percent = int(downloaded * 100 / total)
                            await _safe_edit(status_msg, progress_text("⏳", label, percent))
        return True
    except Exception as e:
        LOG.warning(f"Download with progress failed: {e}")
        return False


async def upload_with_progress(chat_id: int, file_path: Path, caption: str, status_msg: Message, label: str) -> tuple[int, int] | None:
    """Returns (chat_id, message_id) of the sent document, or None on failure."""
    ...
    async def body_generator():
        ...
        yield field("chat_id", str(chat_id))
        if caption:
            yield field("caption", caption)
            yield field("parse_mode", "HTML")
        yield (
        ...
        with file_path.open("rb") as f:
            while True:
                ...
                yield chunk
                now = time.time()
                if now - last_update[0] > 1.5 or sent >= file_size:
                    last_update[0] = now
    ...
    url = f"{_api_base()}/bot{config.bot_token}/sendDocument"
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, data=body_generator(), headers=headers, timeout=aiohttp.ClientTimeout(total=None)) as resp:
                data = await resp.json()
    except Exception as e:
        LOG.warning(f"Upload with progress failed: {e}")
        return None

    if not data.get("ok"):
        LOG.warning(f"sendDocument failed: {data}")
        return None
    result = data["result"]
...
async def single_zip_flow(message: Message, file_id: str, filename: str) -> None:
    status = await message.answer(progress_text("⏳", "𝖣𝗈𝗐𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀 𝖥𝗂𝗅𝖾", 0))
    task_dir = zip_utils.new_task_dir()
    try:
        local_path = task_dir / zip_utils.safe_name(filename)
        ok = await download_with_progress(file_id, local_path, status, "𝖣𝗈𝗐𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀 𝖥𝗂𝗅𝖾")
        if not ok:
        ...
        result = await upload_with_progress(
            message.chat.id, zip_path, f"✅ <code>{zip_path.name}</code>", status, "𝖴𝗉𝗅𝗈𝖺𝖽𝗂𝗇𝗀 𝖹𝗂𝗉 𝖥𝗂𝗅𝖾"
        )
        await status.delete()
        if result:
            await forward_to_dump(*result)
        else:
            await message.answer("❌ <b>Upload failed.</b> Please try again.")
    finally:
...
async def batch_create_callback(cb: CallbackQuery):
    ...
    try:
        ...
        pass
    status = await cb.message.answer(progress_text("⏳", f"𝖣𝗈𝗐𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀 𝖥𝗂𝗅𝖾 1/{len(files)}", 0))
    ...
    try:
        ...
        for i, f in enumerate(files, start=1):
            ...
            ok = await download_with_progress(f["file_id"], local_path, status, f"𝖣𝗈𝗐𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀 𝖥𝗂𝗅𝖾 {i}/{len(files)}")
            if not ok:
                await cb.message.answer(f"❌ Skipped <code>{f['name']}</code> — too large to download.")
                continue
            ordered.append((local_path, f["name"]))
        if not ordered:
            await status.edit_text("❌ <b>None of the queued files could be downloaded.</b>")
            return
        ...
        result = await upload_with_progress(
            cb.message.chat.id, zip_path, "✅ <b>Your ZIP File Is Ready!</b>", status, "𝖴𝗉𝗅𝗈𝖺𝖽𝗂𝗇𝗀 𝖹𝗂𝗉 𝖥𝗂𝗅𝖾"
        )
        await status.delete()
        if result:
            await forward_to_dump(*result)
        else:
            await cb.message.answer("❌ <b>Upload failed.</b> Please try again.")
    finally:
        zip_utils.cleanup_dir(task_dir)
        BATCH_SESSIONS.pop(user_id, None)
...
async def unzip_flow(message: Message, file_id: str, filename: str) -> None:
    status = await message.answer(progress_text("⏳", "𝖣𝗈𝗐𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀 𝖹𝗂𝗉 𝖥𝗂𝗅𝖾", 0))
    task_dir = zip_utils.new_task_dir()
    try:
        zip_path = task_dir / zip_utils.safe_name(filename)
        ok = await download_with_progress(file_id, zip_path, status, "𝖣𝗈𝗐𝗇𝗅𝗈𝖺𝖽𝗂𝗇𝗀 𝖹𝗂𝗉 𝖥𝗂𝗅𝖾")
        if not ok:
        ...
        for i, f in enumerate(real_files, start=1):
            await _safe_edit(status, progress_text("📥", f"𝖴𝗉𝗅𝗈𝖺𝖽𝗂𝗇𝗀 𝖥𝗂𝗅𝖾 {i}/{len(real_files)}", 0))
            result = await upload_with_progress(
                message.chat.id, f, f"📄 <code>{f.name}</code>", status, f"𝖴𝗉𝗅𝗈𝖺𝖽𝗂𝗇𝗀 𝖥𝗂𝗅𝖾 {i}/{len(real_files)}"
            )
            if result:
                await forward_to_dump(*result)
        await status.delete()
...
async def forward_to_dump(chat_id: int, message_id: int) -> None:
    dump_channel = await database.dump_channel_id()
    if not dump_channel:
        return
    try:
        await bot.copy_message(int(dump_channel), chat_id, message_id)
    except Exception as e:
